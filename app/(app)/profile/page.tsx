...
import { ArrowLeft, LogOut, Heart, Settings, Bell } from 'lucide-react';
import { requestNotificationPermission, sendPushNotification } from '@/shared/utils/pwa';
...
export default function ProfilePage() {
  ...
  const fetchProfile = async () => {
    ...
    if (data) {
      ...
      if (data.partner_id) {
        const { data: p } = await supabase.from('profiles').select('*').eq('id', data.partner_id).maybeSingle();
  ...
  const requestNotif = async () => {
    ...
    if (perm === 'granted') {
      await sendPushNotification('MyKiv AI', { body: 'Notifications are now enabled!', icon: '/icon-192x192.png' });
    }
  };
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col items-center gap-3">
        <div className="h-20 w-20 overflow-hidden rounded-full bg-neutral-200">
          {profile?.avatar_url ? <img src={profile.avatar_url} alt="" className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center text-2xl font-bold text-neutral-500">{profile?.full_name?.charAt(0) || 'U'}</div>}
        </div>
      ...
      <Card className="rounded-2xl border-0 p-5 shadow-sm">
        <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold text-neutral-700"><Heart className="h-4 w-4 text-rose-500" /> Partner</h2>
        {partner ? (
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 overflow-hidden rounded-full bg-neutral-200">
              {partner.avatar_url ? <img src={partner.avatar_url} alt="" className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center text-xs font-bold text-neutral-500">{partner.full_name?.charAt(0) || 'P'}</div>}
            </div>
            <div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <Input value={coupleCodeInput} onChange={(e) => setCoupleCodeInput(e.target.value)} placeholder="Enter partner's code" className="h-12 rounded-xl" />
      ...
      <Card className="rounded-2xl border-0 p-5 shadow-sm">
        <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold text-neutral-700"><Bell className="h-4 w-4 text-amber-500" /> Notifications</h2>
        <div className="flex items-center justify-between">
          <span className="text-sm text-neutral-700">Push Notifications</span>
          <Button size="sm" variant={notifPermission === 'granted' ? 'outline' : 'default'} onClick={requestNotif} className="rounded-lg">
      ...
      <Card className="rounded-2xl border-0 p-5 shadow-sm">
        <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold text-neutral-700"><Settings className="h-4 w-4 text-neutral-500" /> Privacy Settings</h2>
        <div className="space-y-4">
          {[
            ...
            <div key={setting.key} className="flex items-center justify-between">
              ...
              <Switch checked={profile?.privacy_settings?.[setting.key as keyof typeof profile.privacy_settings] || false} onCheckedChange={(checked) => updatePrivacy(setting.key, checked)} disabled={saving} />
            </div>
          ))}
        </div>
      </Card>

      <Button variant="outline" onClick={handleSignOut} className="h-12 w-full rounded-xl border-rose-200 text-rose-600 hover:bg-rose-50">
